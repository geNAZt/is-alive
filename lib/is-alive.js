...
util.inherits(IsAlive, events.EventEmitter);
...
IsAlive.prototype.add = function (server, expectedStatusCode, cb) {
    ...
    ((urlParsed.protocol == 'https:') ? https : http).globalAgent.maxSockets += 1; //Increase maxSocket by the amount of servers added
    this._check(server);
};
...
IsAlive.prototype._check = function (server) {
    ...
    request = ((serverObj.url.protocol == 'https:') ? https : http).get(url.format(serverObj.url), function (res) {
        if (serverObj.statusCodes.indexOf(res.statusCode) !== -1) {
            self._setServerTo(server, true);
        } else {
            self._setServerTo(server, false);
        }
    });
...
IsAlive.prototype._setServerTo = function (server, state) {
    "use strict";

    if (this._server[server].isAlive === true && state === false) {
        this.emit("down", server);
        this._server[server].isAlive = false;
    } else if (this._server[server].isAlive === false && state === true) {
        this.emit("alive", server);
        this._server[server].isAlive = true;
    }
};
